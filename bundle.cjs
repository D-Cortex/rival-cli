...
function validateFilesForRuntime(filePaths, runtime) {
...
function readFilesForUpload(filePaths, rootDir = process.cwd()) {
  return filePaths.map((filePath) => {
    const absPath = import_path.default.resolve(rootDir, filePath);
    if (!import_fs.default.existsSync(absPath)) {
      throw new Error(`File not found: ${absPath}`);
    }
    const content = import_fs.default.readFileSync(absPath, "utf-8");
    const relativePath = import_path.default.relative(rootDir, absPath);
    const apiPath = "/" + relativePath.replace(/\\/g, "/");
    const name = import_path.default.basename(relativePath);
    return {
      path: apiPath,
...
function loadProjectConfig(dir = process.cwd()) {
  const configPath = import_path.default.join(dir, "rival.json");
...
function pickTemplate(filename, runtime) {
...
function createAddCommand() {
  ...
  command.description("Add a new file to the project with a starter template").argument("<filename>", "File to create, e.g. sample.py").option("--cwd <dir>", "Working directory (default: current directory)").action((filename, options) => {
    const workDir = options.cwd ?? process.cwd();
    const projectConfig = loadProjectConfig(workDir);
    if (!projectConfig) {
      console.error(
        source_default.red("Error: ") + "rival.json not found. Run `rival init` to set up the project first."
      );
      process.exit(1);
    }
    ...
    const destPath = import_path2.default.join(workDir, filename);
    if (import_fs2.default.existsSync(destPath)) {
      console.error(
        source_default.red("Error: ") + `${filename} already exists. Delete it first or choose a different name.`
      );
      process.exit(1);
    }
    ...
    import_fs2.default.writeFileSync(destPath, content, "utf-8");
    const currentFiles = projectConfig.files ?? [];
    if (!currentFiles.includes(filename)) {
      currentFiles.push(filename);
      const rivalJsonPath = import_path2.default.join(workDir, "rival.json");
      const cfg = JSON.parse(import_fs2.default.readFileSync(rivalJsonPath, "utf-8"));
      cfg.files = currentFiles;
      import_fs2.default.writeFileSync(rivalJsonPath, JSON.stringify(cfg, null, 2), "utf-8");
    }
    console.log(source_default.green("\u2713") + ` Created ${source_default.bold(filename)}`);
    console.log(source_default.dim(`  Added to rival.json files list`));
    console.log(source_default.dim(`  Run \`rival push\` to upload.`));
  });
  return command;
}
...
var CONFIG_DIR = import_path3.default.join(import_os.default.homedir(), ".rival");
var CONFIG_FILE = import_path3.default.join(CONFIG_DIR, "config.json");
function readConfig() {
  try {
    if (!import_fs3.default.existsSync(CONFIG_FILE)) return {};
    return JSON.parse(import_fs3.default.readFileSync(CONFIG_FILE, "utf-8"));
  } catch {
    return {};
  }
}
function writeConfig(config) {
  if (!import_fs3.default.existsSync(CONFIG_DIR)) {
    import_fs3.default.mkdirSync(CONFIG_DIR, { recursive: true });
  }
  import_fs3.default.writeFileSync(CONFIG_FILE, JSON.stringify(config, null, 2), "utf-8");
}
...
function clearConfig() {
  if (import_fs3.default.existsSync(CONFIG_FILE)) {
    import_fs3.default.unlinkSync(CONFIG_FILE);
  }
}
...
// src/lib/api.ts
var DESCOPE_API_URL = "https://api.descope.com";
var DescopeClient = class {
  client;
  ...
  async refreshSession(refreshJwt) {
    const res = await this.client.post("/v1/auth/refresh", { refreshJwt });
    return res.data;
  }
};
var DESCOPE_PROJECT_ID = process.env.DESCOPE_PROJECT_ID ?? "P34BHLas1imEyKtqssBSuha6Hbxu";
var RivalApiClient = class {
  client;
  constructor(baseUrl, token, orgId) {
    this.client = axios_default.create({
      baseURL: baseUrl,
      headers: {
        "Content-Type": "application/json",
  ...
  async getMetadata() {
    const res = await this.client.get("/api/v1/function/public/metadata");
    const body = res.data?.data ?? res.data ?? {};
    return body;
  }
  async getFunctions() {
    const res = await this.client.get("/api/v1/functions/summary");
    const body = res.data?.data ?? res.data;
    return Array.isArray(body) ? body : body?.functions ?? body?.data ?? [];
  }
  async getVersions(orgSlug, fnSlug) {
  ...
  async saveCode(functionId, payload) {
  ...
  async getFunctionVisibility(orgSlug, fnSlug) {
    const res = await this.client.get(
      `/api/v1/function/${encodeURIComponent(orgSlug)}/${encodeURIComponent(fnSlug)}/details`
    );
...
function createFetchCommand() {
  ...
  command.description("List all functions in your organization").option("-u, --api-url <url>", "Override API base URL").action(async (options) => {
    ...
    try {
      ...
      if (functions.length === 0) {
        console.log(source_default.yellow("No functions found in this organization."));
        return;
      }
...
`));
      for (const fn of functions) {
        ...
        console.log(
          source_default.cyan(fn.function_name) + source_default.dim(` (${fn.function_slug})`)
        );
        console.log(
          `  ${source_default.dim("ID:")}       ${fn.function_id}`
        );
        console.log(
          `  ${source_default.dim("Runtime:")}  ${runtime}  ${source_default.dim("Version:")} ${versionName}  ${source_default.dim("State:")} ${state}`
        );
        if (fn.short_description) {
          console.log(`  ${source_default.dim("Desc:")}     ${fn.short_description}`);
        }
        console.log(
          `  ${source_default.dim("Type:")}     ${fn.type ?? "\u2014"}  ${source_default.dim("Visibility:")} ${fn.visibility ?? "\u2014"}`
        );
        console.log("");
      }
    } catch (err) {
      ...
      if (axiosErr?.response?.status === 401) {
        console.error(source_default.dim("Run `rival login` to refresh your session."));
      } else if (axiosErr?.response) {
        console.error(source_default.dim(JSON.stringify(axiosErr.response.data, null, 2)));
      } else {
...
// src/commands/func.ts
var import_fs4 = __toESM(require("fs"));
var import_path4 = __toESM(require("path"));
function parseRivalUri(uri) {
...
function createFuncCommand() {
  const command = new Command("func");
  command.description("Fetch and load a function by its rival:// URI").argument("<uri>", "Function URI, e.g. rival://acme:my-forecast or acme:my-forecast").option("-o, --output <dir>", "Directory to write files into (default: ./<fnSlug>)").option("--version <ver>", "Version to load (default: first available)").option("-u, --api-url <url>", "Override API base URL").action(async (uri, options) => {
    let orgSlug;
    let fnSlug;
    try {
    ...
    const client = new RivalApiClient(apiUrl, token, orgId);
    console.log(
      source_default.dim("rival://") + source_default.cyan(orgSlug) + source_default.dim(":") + source_default.bold(fnSlug) + "\n"
    );
    ...
    try {
      ...
      if (axiosErr?.response?.status === 404) {
        console.error(source_default.red(`Function not found: ${orgSlug}:${fnSlug}`));
        console.error(source_default.dim("Check the org slug and function slug are correct."));
      } else if (axiosErr?.response?.status === 401) {
        console.error(source_default.dim("Session expired. Run `rival login`."));
      } else if (axiosErr?.response) {
        console.error(source_default.dim(JSON.stringify(axiosErr.response.data, null, 2)));
      } else {
        console.error(source_default.red(err instanceof Error ? err.message : String(err)));
    ...
    if (!versions || versions.length === 0) {
      console.error(source_default.yellow("No versions found for this function."));
      process.exit(1);
    }
    const targetLabel = options.version ?? null;
    const versionObj = targetLabel ? versions.find((v) => (v.version || "Draft") === targetLabel) ?? null : versions[0];
    if (!versionObj) {
      console.error(source_default.red(`Version "${targetLabel}" not found.`));
      console.error(
        source_default.dim("Available: ") + versions.map((v) => v.version || "Draft").join(", ")
      );
      process.exit(1);
    }
    ...
    if (files.length === 0) {
      console.log(source_default.yellow(`No files in version "${versionLabel}".`));
      process.exit(0);
    }
    const outDir = import_path4.default.resolve(options.output ?? import_path4.default.join(process.cwd(), fnSlug));
    if (import_fs4.default.existsSync(outDir)) {
      const conflicts = files.map((f) => f.path.replace(/^\//, "")).filter((name) => import_fs4.default.existsSync(import_path4.default.join(outDir, name)));
      if (conflicts.length > 0) {
        console.log(source_default.yellow("\u26A0  The following files already exist and will be overwritten:"));
        for (const f of conflicts) console.log(`   ${source_default.red("M")}  ${f}`);
        console.log();
        const { default: prompts4 } = await Promise.resolve().then(() => __toESM(require_prompts3()));
        const { confirmed } = await prompts4({
          type: "confirm",
          name: "confirmed",
          message: `Overwrite existing files in ${import_path4.default.relative(process.cwd(), outDir) || "."}?`,
          initial: false
        });
        if (!confirmed) {
    ...
    for (const file of files) {
      ...
      const dest = import_path4.default.join(outDir, fileName);
      import_fs4.default.mkdirSync(import_path4.default.dirname(dest), { recursive: true });
      import_fs4.default.writeFileSync(dest, file.data ?? "", "utf-8");
      writtenFiles.push(fileName);
      const lines = (file.data ?? "").split("\n").length;
      console.log(
        source_default.green("\u2713") + "  " + source_default.bold(fileName) + source_default.dim(`  (${file.meta?.mime ?? "text/plain"}, ${lines} lines)`)
      );
    }
    let functionId;
    try {
    ...
    import_fs4.default.writeFileSync(
      import_path4.default.join(outDir, "rival.json"),
      JSON.stringify(rivalJson, null, 2) + "\n",
      "utf-8"
    );
    console.log(
      `
${source_default.green("\u2713")} Loaded ${source_default.bold(fnSlug)} @ ${source_default.cyan(versionLabel)} ` + source_default.dim(`\u2192 ${import_path4.default.relative(process.cwd(), outDir) || "."}`)
    );
    console.log(source_default.dim(`  ${writtenFiles.length} file(s) written  \xB7  rival.json created`));
    console.log(source_default.dim(`
  cd ${import_path4.default.relative(process.cwd(), outDir) || "."} && rival push`));
  });
  return command;
}
...
function createInitCommand() {
  ...
  command.description("Create a new Rival function and scaffold local files").option("-u, --api-url <url>", "Override API base URL").action(async (options) => {
    const cwd = process.cwd();
    if (import_fs5.default.existsSync(import_path5.default.join(cwd, "rival.json"))) {
      console.error(
        source_default.red("Error: ") + "rival.json already exists. Already initialized."
      );
      process.exit(1);
    }
    const token = requireToken();
    ...
    const answers = await (0, import_prompts.default)([
      ...
      }] : [],
      {
        ...
        name: "runtime",
        message: "Runtime:",
        choices: (meta.runtimes ?? []).filter((r) => TEMPLATES2[r.name]).map((r) => ({ title: r.display_name, value: r.name })),
        initial: Math.max(0, (meta.runtimes ?? []).filter((r) => TEMPLATES2[r.name]).findIndex((r) => r.default))
      },
      {
        type: "select",
        name: "categoryId",
    ...
    try {
      ...
      if (!res?.success || !res?.data?.function?.function_id) {
        ...
        console.error(source_default.dim(JSON.stringify(res, null, 2)));
        process.exit(1);
      }
      functionId = res.data.function.function_id;
      spinner.succeed(`Function created \u2014 ID: ${source_default.cyan(functionId)}`);
      const backendFiles = res.data.function.versions?.[0]?.files ?? [];
      const writtenFiles = [];
      for (const file of backendFiles) {
        const fileName = file.path.replace(/^\//, "");
        const filePath = import_path5.default.join(cwd, fileName);
        import_fs5.default.writeFileSync(filePath, file.data, "utf-8");
        console.log(source_default.green("  created  ") + fileName);
        writtenFiles.push(fileName);
      }
      if (writtenFiles.length === 0) {
        const template = TEMPLATES2[answers.runtime] ?? TEMPLATES2["python:3.13"];
        import_fs5.default.writeFileSync(import_path5.default.join(cwd, template.file), template.content, "utf-8");
        console.log(source_default.green("  created  ") + template.file);
      ...
      import_fs5.default.writeFileSync(
        import_path5.default.join(cwd, "rival.json"),
        JSON.stringify(rivalConfig, null, 2) + "\n",
        "utf-8"
      );
      console.log(source_default.green("  created  ") + "rival.json");
      console.log(`
${source_default.bold("Done!")} Edit your files, then run:

  ${source_default.bold("rival push")}
`);
    } catch (err) {
      spinner.fail("Failed to create function on Rival.");
      const axiosErr = err;
      if (axiosErr?.response) {
        console.error(source_default.red(`HTTP ${axiosErr.response.status} \u2014 ${axiosErr?.config?.url ?? ""}`));
        console.error(source_default.dim(JSON.stringify(axiosErr.response.data, null, 2)));
      } else {
        console.error(source_default.red(err instanceof Error ? err.message : String(err)));
      }
...
function createLoadCommand() {
  ...
  command.description("Fetch the latest code from Rival and write files to disk").option("--cwd <dir>", "Working directory (default: current directory)").option("-u, --api-url <url>", "Override API base URL").action(async (options) => {
    const workDir = options.cwd ?? process.cwd();
    const projectConfig = loadProjectConfig(workDir);
    if (!projectConfig?.orgSlug || !projectConfig?.fnSlug) {
      console.error(
        source_default.red("Error: ") + 'rival.json missing "orgSlug" or "fnSlug". Run `rival init` to set up the project.'
      );
      process.exit(1);
    }
    const token = requireToken();
    ...
    try {
      ...
      if (files.length === 0) {
        console.log(source_default.yellow(`No files found in version "${versionLabel}".`));
        return;
      }
      console.log(
        source_default.bold(`
${projectConfig.fnSlug}`) + source_default.dim(` \u2014 version: ${versionLabel}  runtime: ${version.runtime}
`)
      );
      const writtenFiles = [];
      for (const file of files) {
        const fileName = file.path.replace(/^\//, "");
        const dest = import_path6.default.join(workDir, fileName);
        import_fs6.default.mkdirSync(import_path6.default.dirname(dest), { recursive: true });
        import_fs6.default.writeFileSync(dest, file.data ?? "", "utf-8");
        writtenFiles.push(fileName);
        const lines = (file.data ?? "").split("\n").length;
        console.log(
          source_default.green("\u2713") + "  " + source_default.bold(fileName) + source_default.dim(`  (${file.meta?.mime ?? "text/plain"}, ${lines} lines)`)
      ...
      const rivalJsonPath = import_path6.default.join(workDir, "rival.json");
      if (import_fs6.default.existsSync(rivalJsonPath)) {
        const cfg = JSON.parse(import_fs6.default.readFileSync(rivalJsonPath, "utf-8"));
        cfg.files = writtenFiles;
        import_fs6.default.writeFileSync(rivalJsonPath, JSON.stringify(cfg, null, 2), "utf-8");
      }
      console.log(source_default.dim(`
...
function createLoginCommand() {
  ...
  command.description("Log in to Rival using your email (OTP via Descope)").option("-e, --email <email>", "Your Rival account email").option("-u, --api-url <url>", "Override API base URL").action(async (options) => {
    ...
    try {
      ...
      const sendSpinner = ora(`Sending OTP to ${source_default.cyan(email)}\u2026`).start();
      await descope.sendOtp(email);
      sendSpinner.succeed(`OTP sent to ${source_default.cyan(email)}`);
      const { code } = await (0, import_prompts2.default)({
        type: "text",
        name: "code",
        message: "Enter the OTP code:",
        validate: (v) => v.trim().length > 0 || "OTP cannot be empty"
      });
      ...
      if (!token) {
        ...
        console.error(source_default.red("Error: ") + "No sessionJwt in Descope response.");
        process.exit(1);
      }
      ...
      if (orgs.length === 0) {
        console.error(
          source_default.red("Error: ") + "No organizations found. Create one at rival.io first."
        );
        process.exit(1);
      }
      let orgId;
      if (orgs.length === 1) {
        ...
        console.log(
          source_default.green("\u2713") + ` Using organization: ${source_default.cyan(orgs[0].organization_name)}`
        );
      } else {
        const { selectedOrg: selectedOrg2 } = await (0, import_prompts2.default)({
          type: "select",
          name: "selectedOrg",
          message: "Select your organization:",
          choices: orgs.map((org) => ({
            title: `${org.organization_name} ${source_default.dim(`(${org.organization_slug})`)}`,
            value: org.organization_id
          }))
        });
        if (!selectedOrg2) process.exit(0);
        orgId = selectedOrg2;
      }
      writeConfig({ token, refreshToken, orgId, email, apiUrl });
      const selectedOrg = orgs.find((o) => o.organization_id === orgId);
      console.log("\n" + source_default.green("\u2713") + " Logged in as " + source_default.cyan(email));
      console.log(source_default.dim(`  Org: ${selectedOrg?.organization_name} (${orgId})`));
...
HTTP ${axiosErr.response.status ?? "?"} \u2014 ${axiosErr?.config?.url ?? ""}`));
        console.error(source_default.dim("Request headers: ") + JSON.stringify(axiosErr?.config?.headers, null, 2));
        console.error(source_default.dim("Response body:   ") + JSON.stringify(axiosErr.response.data, null, 2));
      } else {
        const message = err instanceof Error ? err.message : String(err);
...
function createLogoutCommand() {
  ...
  command.description("Clear saved credentials").action(() => {
    clearConfig();
    console.log(source_default.green("\u2713") + " Logged out. Credentials cleared.");
  });
  return command;
}
...
function createPushCommand() {
  ...
  command.description("Push local code files to a Rival function").argument("[files...]", "Files to push (e.g. handler.py utils.py)").option("-f, --function-id <id>", "Function ID to push to").option("-v, --version <version>", "Version to push to (skips prompt)").option("-a, --asset-id <id>", "Digital asset ID (Storm functions only)").option("-u, --api-url <url>", "Override API base URL").option("--cwd <dir>", "Working directory (default: current directory)").action(async (fileArgs, options) => {
    const workDir = options.cwd ?? process.cwd();
    try {
      const projectConfig = loadProjectConfig(workDir);
      const functionId = options.functionId ?? projectConfig?.functionId;
      if (!functionId) {
        console.error(source_default.red("Error: ") + 'Function ID is required. Pass --function-id or add "functionId" to rival.json.');
        process.exit(1);
      }
      ...
      if (fileList.length === 0) {
        console.error(source_default.red("Error: ") + 'No files specified. Pass file paths or add "files" to rival.json.');
        process.exit(1);
      }
      const token = requireToken();
      const orgId = requireOrgId();
      ...
      if (projectConfig?.orgSlug && projectConfig?.fnSlug) {
        ...
        try {
          ...
          if (visibility !== "private") {
            console.error(
              "\n" + source_default.red("Push blocked: ") + `This function is ${source_default.yellow(visibility)}.`
            );
            console.error(
              source_default.dim("Only private functions can be edited via the CLI.")
            );
            process.exit(1);
          }
        } catch {
      ...
      console.log("");
      console.log(source_default.bold("Function:"), source_default.cyan(functionId));
      console.log(source_default.bold("Version: "), source_default.cyan(version));
      console.log(source_default.bold("Files:"));
      files.forEach((f) => console.log(`  ${source_default.dim(f.path)}  (${f.meta.mime})`));
      console.log("");
      ...
      pushSpinner.succeed(source_default.green("Code pushed successfully!"));
      if (result && typeof result === "object") {
        console.log("\n" + source_default.dim(JSON.stringify(result, null, 2)));
      }
    } catch (err) {
      const axiosErr = err;
      if (axiosErr?.response?.status === 401) {
        console.error("\n" + source_default.red("401 Unauthorized") + " \u2014 your session has expired.");
        console.error(source_default.dim("Run `rival login` to get a fresh token, then try again."));
      } else if (axiosErr?.response) {
        console.error("\n" + source_default.red(`HTTP ${axiosErr.response.status} \u2014 ${axiosErr?.config?.url ?? ""}`));
        console.error(source_default.dim(JSON.stringify(axiosErr.response.data, null, 2)));
      } else {
        console.error("\n" + source_default.red("Error: ") + (err instanceof Error ? err.message : String(err)));
      }
      process.exit(1);
    }
  });
...
function writeVersionFiles(files, workDir) {
  const written = [];
  for (const file of files) {
    const fileName = file.path.replace(/^\//, "");
    const dest = import_path7.default.join(workDir, fileName);
    import_fs7.default.mkdirSync(import_path7.default.dirname(dest), { recursive: true });
    import_fs7.default.writeFileSync(dest, file.data ?? "", "utf-8");
    written.push(fileName);
  }
  return written;
}
function detectDirtyFiles(files, workDir) {
  const dirty = [];
  for (const file of files) {
    const fileName = file.path.replace(/^\//, "");
    const diskPath = import_path7.default.join(workDir, fileName);
    if (!import_fs7.default.existsSync(diskPath)) continue;
    const diskContent = import_fs7.default.readFileSync(diskPath, "utf-8");
...
function createSelectCommand() {
  ...
  command.description("Switch to a version and pull its code to disk \u2014 like git checkout").option("--cwd <dir>", "Working directory (default: current directory)").option("-u, --api-url <url>", "Override API base URL").action(async (options) => {
    const workDir = options.cwd ?? process.cwd();
    const configPath = import_path7.default.join(workDir, "rival.json");
    const projectConfig = loadProjectConfig(workDir);
    if (!projectConfig?.orgSlug || !projectConfig?.fnSlug) {
      console.error(
        source_default.red("Error: ") + 'rival.json missing "orgSlug" or "fnSlug". Run `rival init` to set up the project.'
    ...
    if (versions.length === 0) {
      console.log(source_default.yellow('No versions found. Defaulting to "Draft".'));
      projectConfig.version = "Draft";
      import_fs7.default.writeFileSync(configPath, JSON.stringify(projectConfig, null, 2) + "\n", "utf-8");
      process.exit(0);
    }
    const { selectedVersion } = await (0, import_prompts3.default)({
      type: "select",
      name: "selectedVersion",
      message: "Select version:",
      choices: versions.map((v) => {
        const label = v.version || "Draft";
        return { title: `${label}  ${source_default.dim(`(${v.state})`)}`, value: label };
      }),
    ...
    if (selectedVersion === projectConfig.version) {
      console.log(source_default.dim(`Already on version "${selectedVersion}". Nothing changed.`));
      process.exit(0);
    }
    ...
    if (dirty.length > 0) {
      console.log(
        "\n" + source_default.yellow("\u26A0  Uncommitted local changes will be overwritten:")
      );
      for (const f of dirty) {
        console.log(`   ${source_default.red("M")}  ${f}`);
      }
      console.log();
      const { confirmed } = await (0, import_prompts3.default)({
        type: "confirm",
        name: "confirmed",
        message: `Discard local changes and switch to ${source_default.cyan(selectedVersion)}?`,
        initial: false
      });
      if (!confirmed) {
        console.log(source_default.dim("Aborted. Your files are unchanged."));
        process.exit(0);
      }
    }
    const writeSpinner = ora(`Checking out ${source_default.cyan(selectedVersion)}\u2026`).start();
    ...
    import_fs7.default.writeFileSync(configPath, JSON.stringify(projectConfig, null, 2) + "\n", "utf-8");
    console.log(source_default.green("\u2713") + ` Switched to version ${source_default.cyan(selectedVersion)}
`);
    for (const f of written) {
      console.log(`  ${source_default.dim("updated")}  ${f}`);
    }
    if (written.length === 0) {
      console.log(source_default.dim("  (no files in this version)"));
    }
    console.log();
  });
  return command;
...
function createWhoamiCommand() {
  ...
  command.description("Show current login state and verify token with the API").option("-u, --api-url <url>", "Override API base URL").action(async (options) => {
    const config = readConfig();
    if (!config.token) {
      console.log(source_default.yellow("Not logged in.") + " Run `rival login` to authenticate.");
      return;
    }
    const apiUrl = options.apiUrl ?? getApiUrl();
    ...
    try {
      ...
      console.log(`  ${source_default.dim("Email:")}   ${source_default.cyan(user?.email ?? config.email ?? "\u2014")}`);
      console.log(`  ${source_default.dim("Name:")}    ${user?.first_name ?? ""} ${user?.last_name ?? ""}`.trimEnd());
      console.log(`  ${source_default.dim("Org ID:")}  ${source_default.cyan(config.orgId ?? "\u2014")}`);
      console.log(`  ${source_default.dim("API:")}     ${source_default.dim(apiUrl)}`);
      const orgs = user?.organizations ?? [];
      if (orgs.length > 0) {
        console.log(`
  ${source_default.dim("Organizations:")}`);
        orgs.forEach((org) => {
          const active = org.organization_id === config.orgId;
          console.log(
            `    ${active ? source_default.green("\u25CF") : source_default.dim("\u25CB")}  ${org.organization_name}` + source_default.dim(` (${org.organization_slug})`) + (active ? source_default.green("  \u2190 active") : "")
          );
        });
      }
    } catch (err) {
      ...
      const axiosErr = err;
      if (axiosErr?.response?.status === 401) {
        console.log(source_default.red("\u2717") + " Session expired.");
        console.log(source_default.dim("  Run `rival login` to get a fresh token."));
      } else {
        console.log(source_default.red("\u2717") + " Could not verify session.");
        console.log(source_default.dim("  " + (err instanceof Error ? err.message : String(err))));
      }
      process.exit(1);
    }
  });
...
// src/index.ts
import_dotenv.default.config({ path: import_path8.default.join(import_os2.default.homedir(), ".rival", ".env") });
import_dotenv.default.config({ path: import_path8.default.join(__dirname, "../.env") });
var program2 = new Command();
program2.name("rival").description("Rival CLI \u2014 push function code and manage your Rival workspace").version("1.0.0", "-v, -V, --version", "Output the version number");
program2.addCommand(createAddCommand());
program2.addCommand(createFetchCommand());
program2.addCommand(createFuncCommand());
program2.addCommand(createInitCommand());
program2.addCommand(createLoadCommand());
program2.addCommand(createLoginCommand());
program2.addCommand(createLogoutCommand());
program2.addCommand(createPushCommand());
program2.addCommand(createSelectCommand());
